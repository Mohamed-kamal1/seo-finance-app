...
select
  date_trunc('month', actual_date)::date as month,
  classification_is as category,
  sum(debit)  as total_in,
  sum(credit) as total_out,
  sum(debit) - sum(credit) as net
from transactions
where classification_is is not null
group by 1, 2
order by 1, 2;
...
select
  date_trunc('month', coalesce(cf_date, actual_date))::date as month,
  classification_cf as category,
  sum(debit)  as total_in,
  sum(credit) as total_out,
  sum(debit) - sum(credit) as net
from transactions
where classification_cf is not null
group by 1, 2
order by 1, 2;
...
group by t.id, t.name, t.currency_code, t.opening_balance;
...
select
  date_trunc('month', actual_date)::date as month,
  sum(debit)  as revenue,
  sum(credit) as expenses,
  sum(debit) - sum(credit) as net
from transactions
group by 1
order by 1;
